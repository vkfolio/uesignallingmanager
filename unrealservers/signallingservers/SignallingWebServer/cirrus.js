...
const argv = require('yargs')
	.option('UseMatchmaker', {
		type: 'boolean',
		description: 'Whether to use the matchmaker',
		default: false
	})
	.option('MatchmakerAddress', {
		type: 'string',
		description: 'The address of the matchmaker',
		default: '127.0.0.1'
	})
	.option('MatchmakerPort', {
		type: 'number',
		description: 'The port of the matchmaker',
		default: 9999
	})
	.option('PublicIp', {
		type: 'string',
		description: 'Public IP address to use',
		default: 'localhost'
	})
	.option('HttpPort', {
		type: 'number',
	...
	.option('StreamerPort', {
		type: 'number',
		description: 'Port for the streamer',
		default: 8888
	})
	.option('UseHTTPS', {
		type: 'boolean',
...
// Config file path, fallback to default if not provided
var configFile = (typeof argv.configFile != 'undefined') ? argv.configFile.toString() : path.join(__dirname, 'config.json');
console.log(`Config file: ${configFile}`);

// Initialize config from file, overriding with command-line arguments
const config = Object.assign({}, defaultConfig, require('./modules/config.js').init(configFile, defaultConfig), argv);

// Log configuration after merging
